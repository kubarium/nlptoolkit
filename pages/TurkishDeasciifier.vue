...
async function spellCheck(sentence: string) {
  ...
  const { data } = await useFetch(`/api/TurkishDeasciifier?sentence=${sentence}`)
  searchResult.value = data.value.payload
}
</script>
<template>
  <div class="flex flex-col gap-6 p-6 h-screen">
    <h1 class="text-center text-xl text-sky-600">Turkish Deasciifier</h1>
    ...
    <template v-if="search">
      <div class="flex flex-col items-center">
        <div class="text-lg my-2">Deasciified version of</div>
        <div class="">
          {{ search }}
        </div>
